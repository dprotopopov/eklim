...
On Error GoTo errMyErrorHandler
    ...
    Dim lyr5 As Layer ' PICTURE
    Dim lyr6 As Layer ' WHITE
    Dim lyr7 As Layer ' RGB
    Dim rgb As Color, rgb1 As Color
    Dim cmyk As Color
    Dim export As StructExportOptions
    Dim expflt As ExportFilter
    Dim import As StructImportOptions
...
'1.       Отобразить экранную форму
    ...
    myFileNameText = "D:\Projects\eklim\имена.v3.xls"
    myFileNameImage = "D:\Projects\eklim\cases-uv.xls"
    ...
    myMessageImageFolderName = "D:\Projects\eklim\backgrounds"
    myPictureImageFolderName = "D:\Projects\eklim\pictures"
    
'    myFileNameText = "E:\name script\имена.xls"
'    myFileNameImage = "E:\name script\cases-uv.xls"
'    mySaveFolderName = "E:\name script\makets"
'    myExcelFolderName = "E:\name script\makets"
'    myExcelFileName = "E:\name script\заказ.xlt"
'    myMessageImageFolderName = "E:\name script\backgrounds"
'    myPictureImageFolderName = "E:\name script\pictures"
...
While Trim(CStr(cells(ROW_COUNTER1 + 2, 1))) <> "" _
Or Trim(CStr(cells(ROW_COUNTER1 + 2, 2))) <> "" _
Or Trim(CStr(cells(ROW_COUNTER1 + 2, 3))) <> ""
    If Trim(CStr(cells(ROW_COUNTER1 + 2, 1))) <> "" Then
        dateString = Trim(CStr(cells(ROW_COUNTER1 + 2, 1)))
    End If
    xls(ROW_COUNTER, 0) = dateString ' дата
    xls(ROW_COUNTER, 1) = Trim(CStr(cells(ROW_COUNTER1 + 2, 2))) '    номер заказа
    xls(ROW_COUNTER, 2) = Trim(CStr(cells(ROW_COUNTER1 + 2, 3))) '     модель
    xls(ROW_COUNTER, 3) = Trim(CStr(cells(ROW_COUNTER1 + 2, 4))) '   цвет чехла
    xls(ROW_COUNTER, 4) = ""
    xls(ROW_COUNTER, 5) = Trim(CStr(cells(ROW_COUNTER1 + 2, 5))) '   надпись
    xls(ROW_COUNTER, 6) = Trim(CStr(cells(ROW_COUNTER1 + 2, 6))) '  надпись 2
    xls(ROW_COUNTER, 7) = Trim(CStr(cells(ROW_COUNTER1 + 2, 7))) '   Шаблон
    xls(ROW_COUNTER, 8) = Trim(CStr(cells(ROW_COUNTER1 + 2, 8))) '   цвет букв(C, M, Y, K)
    xls(ROW_COUNTER, 9) = Trim(CStr(cells(ROW_COUNTER1 + 2, 9))) '   логотип
    xls(ROW_COUNTER, 10) = Trim(CStr(cells(ROW_COUNTER1 + 2, 10))) '  количество
    xls(ROW_COUNTER, 11) = Trim(CStr(cells(ROW_COUNTER1 + 2, 11))) '  статус  примечание  комментарий менеджера
...
'Now open the Excel file that is located in the same folder as this CorelDraw fie.
Workbooks.Open (myFileNameImage)
...
'This is because the first row of the spreadsheet is a header & so is not read.
'If you do not have a header use cells(ROW_COUNTER,...... in place of cells(ROW_COUNTER,..... ie Do not use + 1
While Trim(CStr(cells(ROW_COUNTER2 + 2, 1))) <> "" _
Or Trim(CStr(cells(ROW_COUNTER2 + 2, 2))) <> "" _
Or Trim(CStr(cells(ROW_COUNTER2 + 2, 3))) <> ""
    xls(ROW_COUNTER, 0) = dateString
    xls(ROW_COUNTER, 1) = Trim(CStr(cells(ROW_COUNTER2 + 2, 1))) ' № заказа
    xls(ROW_COUNTER, 2) = Trim(CStr(cells(ROW_COUNTER2 + 2, 2))) '    Модель
    xls(ROW_COUNTER, 3) = Trim(CStr(cells(ROW_COUNTER2 + 2, 3))) '   Цвет чехла
    xls(ROW_COUNTER, 4) = Trim(CStr(cells(ROW_COUNTER2 + 2, 4))) '   Макет
    xls(ROW_COUNTER, 5) = ""
    xls(ROW_COUNTER, 6) = ""
    xls(ROW_COUNTER, 7) = ""
    xls(ROW_COUNTER, 8) = ""
    xls(ROW_COUNTER, 9) = ""
    xls(ROW_COUNTER, 10) = Trim(CStr(cells(ROW_COUNTER2 + 2, 5))) '    Количество
    xls(ROW_COUNTER, 11) = "" '  Комментарий
    ROW_COUNTER = ROW_COUNTER + 1
...
If ORDERDATE_COUNTER = 0 Then Err.Raise vbObjectError, "myFileNameText", "Нет списка заказов"
...
'удаление дпликатов из ORDERDATE
...
For m = 0 To ORDERDATE_COUNTER - 1
    ...
    For row = 0 To ROW_COUNTER - 1
        ...
        And CStr(xls(row, 7)) <> "" Then
            ...
            PICTURE(COUNTER) = CStr(xls(row, 4))
            MESSAGE(COUNTER) = CStr(xls(row, 5))
            SUBMESSAGE(COUNTER) = CStr(xls(row, 6))
            MESSAGETEMPLATE(COUNTER) = CStr(xls(row, 7))
            LABELCOLOR(COUNTER) = CStr(xls(row, 8))
            LOGOTYPE(COUNTER) = CStr(xls(row, 9))
            AMOUNT(COUNTER) = CStr(xls(row, 10))
            LOGOTYPETEMPLATE(COUNTER) = CStr(xls(row, 11))
...
If MODEL_COUNTER = 0 Then Err.Raise vbObjectError, "myFileNameText", "Нет списка моделей"
...
'b.      Берём лист с совпадающим именем
    ...
    For i = 0 To MODELCOLOR_COUNTER - 1
        ...
        For row = 0 To ROW_COUNTER - 1
            ...
            And UCase(xls(row, 3)) = UCase(MODELCOLOR1(i)) Then
                ...
                If PICTURE(COUNTER) + MESSAGE(COUNTER) + SUBMESSAGE(COUNTER) = "" Then Err.Raise vbObjectError, model(m), "Не задана MESSAGE в строке " + CStr(row + 1)
                If MESSAGE(COUNTER) + SUBMESSAGE(COUNTER) <> "" _
                And LABELCOLOR(COUNTER) = "" Then Err.Raise vbObjectError, model(m), "Не задана LABELCOLOR в строке " + CStr(row + 1)
                If MESSAGE(COUNTER) + SUBMESSAGE(COUNTER) <> "" _
                And MESSAGEFONT(COUNTER) = "" Then Err.Raise vbObjectError, model(m), "Не задана MESSAGEFONT в строке " + CStr(row + 1)
                If MESSAGE(COUNTER) + SUBMESSAGE(COUNTER) <> "" _
                And LOGOTYPEFONT(COUNTER) = "" Then Err.Raise vbObjectError, model(m), "Не задана LOGOTYPEFONT в строке " + CStr(row + 1)
    ...
    Next i
        ...
        Do
            ...
            Set lyr7 = FindOrCreateLayer(newPage, "RGB")
            Set lyr6 = FindOrCreateLayer(newPage, "WHITE")
            Set lyr5 = FindOrCreateLayer(newPage, "PICTURE")
            ...
            Set sr5 = lyr5.FindShapes(Type:=cdrCurveShape) ' PICTURE
            ...
            If t < sr5.Count Then t = sr5.Count ' PICTURE
            ...
            If t > sr5.Count And sr5.Count > 0 Then t = sr5.Count ' PICTURE
            ...
            For j = 1 To t
                If k < COUNTER And CInt(AMOUNT(k)) > 0 Then
                    ...
                    file = myPICTUREIMAGE(model(m), PICTURE(k))
                    
                    If fs.FileExists(file) Then
                    
                        symname = myPICTURESYMBOL(model(m), PICTURE(k))
                        
                        If FindSymbol(syms, symname) Is Nothing Then
                            Set lyr = FindOrCreateLayer(newPage, "IMPORT")
                            ' регистрируем символ
                            Set import = CreateStructImportOptions
                            With import
                                .Mode = cdrImportFull
                                .MaintainLayers = True
                                ...
                                End With
                            End With
                            Set impflt = lyr.ImportEx(file, cdrPNG, import)
                            With impflt
                                .Finish
                            End With
                        ...
                        If j <= sr5.Count Then
                             ...
                             Set s = lyr0.FindShape(Type:=cdrCurveShape) ' PICTURE
                    ...
                    file = myMESSAGEIMAGE(model(m), LABELCOLOR(k))
                    
                    If fs.FileExists(file) Then
                    
                        symname = myMESSAGESYMBOL(model(m), LABELCOLOR(k))
                    ...
                    ElseIf MESSAGE(k) + SUBMESSAGE(k) <> "" Then
                        ...
                        If title <> "" Then
                            ...
                            If j <= sr.Count Then
                                With sr(j) ' MESSAGE
                                    ...
                                    lyr0.Paste ' MESSAGE
                                End With
                                If Not rgb1.IsSame(myRgbWhite) Then lyr6.Paste ' WHITE
                            End If
                        End If
                        ...
                        If j <= sr1.Count Then sr1(j).Delete
                        If j <= sr2.Count Then sr2(j).Delete
                        If j <= sr5.Count Then sr5(j).Delete
                    End If
...
On Error Resume Next
            For Each s In lyr6.Shapes.All  ' WHITE
                With s
...
Next m
    ...
    For Each newPage In newDoc.Pages
        For Each lyr In newPage.Layers
            ...
            If lyr.name = "PICTURE" Then lyr.visible = True
            ...
            If lyr.name = "PICTURE" Then lyr.Printable = True
